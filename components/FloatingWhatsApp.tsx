    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="fixed bottom-[max(1.25rem,env(safe-area-inset-bottom))] left-4 z-50 flex items-center gap-3 md:bottom-8 md:left-[calc(min(5vw,6rem)+1rem)]"
        >
          <motion.a
          ...
          {/* Balãozinho de fala (à direita do botão) */}
          <AnimatePresence>
            {bubble && (
              <motion.button
                type="button"
                ...
                className="relative rounded-2xl rounded-bl-sm bg-white px-4 py-2.5 font-display text-sm font-semibold text-cocoa shadow-card ring-1 ring-cocoa/10"
              >
                Bateu a fome? Peça aqui! 🍌
                <span className="absolute -left-1.5 bottom-2 h-3 w-3 rotate-45 bg-white ring-1 ring-cocoa/10 [clip-path:polygon(0_0,0_100%,100%_100%)]" />
              </motion.button>
            )}
          </AnimatePresence>
        </motion.div>
